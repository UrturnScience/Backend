...
test.serial(
  ...
  async t => {
    ...
    const preference61 = await create_models.create_preference(chore6.id, user1.id, 53); //Test preference checking/fixing
    const preference62 = await create_models.create_preference(chore6.id, user2.id, 32);
    const preference63 = await create_models.create_preference(chore6.id, user3.id, 23);

    const res2 = await request(app).post("/assignment/assignmentCycle").expect(200);
