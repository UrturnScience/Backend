...
test.serial("PUT /assignment/active/:id", async t => {
  ...
  assignment1.active = false;
  assignment1.save(); //active = false

  let assignment2 = await create_models.create_assignment(chore.id, user.id); //active = true

  const res1 = await request(app)
    .put("/assignment/active/" + assignment1.id)
    .expect(200);

  const res2 = await request(app)
    .put("/assignment/active/" + assignment2.id)
    .expect(200);
  ...
  t.truthy(assignment1.active == true);
  t.truthy(assignment2.active == false);
});
