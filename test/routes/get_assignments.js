...
test.serial(
  "GET advanced gets for assignment/chore/preferences",
  async t => {
    ...
    const chore1 = await create_models.create_chore(room1.id, "Dishes", 5, recurring = true, upcoming = true);
    const chore2 = await create_models.create_chore(room1.id, "Laundry", 3, recurring = false, upcoming = true);
    const chore3 = await create_models.create_chore(room1.id, "Mow", 8, recurring = false, upcoming = true);
    const chore4 = await create_models.create_chore(room1.id, "Clean House", 13, recurring = true,  upcoming = true);
    const chore5 = await create_models.create_chore(room1.id, "Scrub", 15, recurring = true, upcoming = false);
    ...
    const preference13 = await create_models.create_preference(chore1.id, user3.id, 1);
    ...
    const preference23 = await create_models.create_preference(chore2.id, user3.id, 3);
    ...
    const preference33 = await create_models.create_preference(chore3.id, user3.id, 2);

    const preference41 = await create_models.create_preference(chore4.id, user1.id, 2);
    const preference42 = await create_models.create_preference(chore4.id, user2.id, 0);
    const preference43 = await create_models.create_preference(chore4.id, user3.id, 0);

    const preference51 = await create_models.create_preference(chore5.id, user1.id, 2);
    const preference52 = await create_models.create_preference(chore5.id, user2.id, 0);
    const preference53 = await create_models.create_preference(chore5.id, user3.id, 0);
    ...
    //Check get routes
    const res_upcomingChores1 = await request(app).get("/chore/upcoming/" + room1.id).expect(200);
    const res_activeChores1 = await request(app).get("/chore/active/" + room1.id).expect(200);
    const res_inactiveAssignments11 = await request(app).get("/assignment/inactive/" + user1.id).expect(200);
    const res_activeAssignments11 = await request(app).get("/assignment/active/" + user1.id).expect(200);
    const res_inactiveAssignments12 = await request(app).get("/assignment/inactive/" + user2.id).expect(200);
    const res_activeAssignments12 = await request(app).get("/assignment/active/" + user2.id).expect(200);
    const res_preferences11 = await request(app).get("/preference/upcoming/" + user1.id).expect(200);
    ...
    const res_upcomingChores2 = await request(app).get("/chore/upcoming/" + room1.id).expect(200);
    const res_activeChores2 = await request(app).get("/chore/active/" + room1.id).expect(200);
    const res_inactiveAssignments21 = await request(app).get("/assignment/inactive/" + user1.id).expect(200);
    const res_activeAssignments21 = await request(app).get("/assignment/active/" + user1.id).expect(200);
    const res_inactiveAssignments22 = await request(app).get("/assignment/inactive/" + user2.id).expect(200);
    const res_activeAssignments22 = await request(app).get("/assignment/active/" + user2.id).expect(200);
    const res_preferences21 = await request(app).get("/preference/upcoming/" + user1.id).expect(200);
    ...
    const res2 = await request(app).put("/assignment/retireAssignments").expect(200);
    ...
    //Check get routes again
    const res_upcomingChores3 = await request(app).get("/chore/upcoming/" + room1.id).expect(200);
    const res_activeChores3 = await request(app).get("/chore/active/" + room1.id).expect(200);
    const res_inactiveAssignments31 = await request(app).get("/assignment/inactive/" + user1.id).expect(200);
    const res_activeAssignments31 = await request(app).get("/assignment/active/" + user1.id).expect(200);
    const res_inactiveAssignments32 = await request(app).get("/assignment/inactive/" + user2.id).expect(200);
    const res_activeAssignments32 = await request(app).get("/assignment/active/" + user2.id).expect(200);
    const res_preferences31 = await request(app).get("/preference/upcoming/" + user1.id).expect(200);
